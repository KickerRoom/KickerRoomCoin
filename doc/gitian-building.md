...
*Setup instructions for a Gitian build of KickerRoom Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the KickerRoom
...
- [Building KickerRoom Core](#building-kickerroom-core)
...
Clone the git repositories for KickerRoom Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/kickerroomcrypto/kickerroom
```
...
in the KickerRoom Core repository under 'Fetch and build inputs' to install sources which require
...
To build KickerRoom Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the KickerRoom Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kickerroom/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/kickerroomcrypto/kickerroom
...
For example:
```bash
URL=https://github.com/crowning-/kickerroom.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit kickerroom=${COMMIT} --url kickerroom=${URL} ../kickerroom/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kickerroom=${COMMIT} --url kickerroom=${URL} ../kickerroom/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kickerroom=${COMMIT} --url kickerroom=${URL} ../kickerroom/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the kickerroom git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kickerroom/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/kickerroom.git
SIGPATH=/some/root/path/kickerroom-detached-sigs.git

./bin/gbuild --url kickerroom=${BTCPATH},signature=${SIGPATH} ../kickerroom/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kickerroom-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kickerroom-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kickerroom-osx-build.assert
```
...
[kickerroom/gitian.sigs](https://github.com/kickerroomcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
