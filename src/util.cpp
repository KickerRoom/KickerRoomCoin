...
const char * const BITCOIN_CONF_FILENAME = "kickerroom.conf";
const char * const BITCOIN_PID_FILENAME = "kickerroomd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "kickerroom" is a composite category enabling all KickerRoom-related debug output
            if(ptrCategory->count(string("kickerroom"))) {
...
#else
    const char* pszModule = "kickerroom";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KickerRoomCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KickerRoomCore
    // Mac: ~/Library/Application Support/KickerRoomCore
    // Unix: ~/.kickerroomcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KickerRoomCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KickerRoomCore";
#else
    // Unix
    return pathRet / ".kickerroomcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty kickerroom.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override kickerroom.conf
